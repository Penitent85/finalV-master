...
const DoctorProfile = () => {
    ...
    const { dToken, profileData, setProfileData, getProfileData  , doctorId} = useContext(DoctorContext)
    ...
    const updateProfile = async () => {
        ...
        try {
            ...
            const { data } = await axios.post(backendUrl + `api/doctor/${doctorId}`, updateData, {
                headers: {
    ...
    return profileData && (
        <div>
            <div className='flex flex-col gap-4 m-5'>
                <div>
                    <Image className='bg-primary/80 w-full sm:max-w-64 rounded-lg' src={images} alt="" />
                </div>
